...
class Decoder(nn.Module):
    ...
    def __init__(self,latent_size) -> None:
        ...
        self.begin = nn.Sequential(
            nn.Linear(self.latent_size+10,(self.begin_size*self.begin_size)*64),
            nn.ReLU()
        )
        self.net = nn.Sequential(
            nn.ConvTranspose2d(in_channels=64,out_channels=64,kernel_size=3,padding=1),
            nn.ReLU(),
            nn.ConvTranspose2d(in_channels=64,out_channels=32,kernel_size=4,padding=1),
            nn.ReLU(),
            nn.ConvTranspose2d(in_channels=32,out_channels=16,kernel_size=4,padding=1,stride=2),
            nn.ReLU(),
            nn.ConvTranspose2d(in_channels=16,out_channels=1,kernel_size=5,padding=1),
        )
...
class VAE(nn.Module):
    ...
    def get_loss(self,x,y):
        ...
        reconstruct = torch.sum((self.decoder(sample,y)-x.squeeze(1))**2,dim=[1,2])/(2*self.x_std)
