...
def train(epochs,model:Transformer,wmt:WMT19,optimizer,eval_num=2000):
    ...
    for epoch in range(epochs):
        model.train()
        losses = []
        with tqdm(train_loader,total=eval_num) as bar:
            for i,(src,tgt) in enumerate(bar):
                ...
                if i % 10 == 0:
                    bar.update(10)
                    bar.set_description(f'Epoch {i/leng+epoch+1:.4f}: Loss {sum(losses[-10:])/len(losses[-10:]):.4f}')
